$(function(){
    $("#incluir").click(function(){
        $("#modal-inclusao").modal("show");
    });

    $(".cancelar").click(function(){
        $(".modal").modal("hide");
    });

    $("#inserir").click(function(){
        let acao = "inserir";
        let nome = $("#inserir_nome").val();
        let descricao = $("#inserir_descricao").val();
        let imagem = $("#inserir_imagem").val();
        let tipo = $("#inserir_tipo").val();
        let faccao = $("#inserir_faccao").val();
        let ativo = $("#inserir_ativo").is(":checked") ? 1 : 0;

        $.ajax({
            type:"POST",
            url:"personagem-model.php",
            data:"acao="+acao+"&nome="+nome+"&descricao="+descricao+"&imagem="+imagem+"&tipo="+tipo+"&faccao="+faccao+"&ativo="+ativo,
            success:function(msg){
                alert(msg);

                $("#modal-inclusao").modal('hide');

                window.location.reload();
            }
        })
    });

    $(".excluir").click(function(){
        let acao = 'excluir'
        let ID = $(this).attr("id");
        let nome = $(this).attr("nome");

        if(confirm("Confirma a desativar '"+nome+"'?")){
            $.ajax({
                type:"GET",
                url:"personagem-model.php",
                data:"acao="+acao+"&ID="+ID,
                success: function(msg){
                    alert(msg);

                    window.location.reload();
                }
            });
        }
    });

    $(".editar").click(function(){
        $("#modal-titulo-edicao").css("color","black");

        let acao = 'editar';
        let ID = $(this).attr("id");

        $.ajax({
            type:"GET",
            url:"personagem-model.php",
            data:"acao="+acao+"&ID="+ID,
            success: function(msg){
                $("#modal-edicao").modal('show');
                $("#modal-corpo-edicao").html(msg);
            }
        });
    });

    $("#alterar").click(function(){
        let acao = 'alterar';
        let ID = $("#editar_ID").val();
        let nome = $("#editar_nome").val();
        let descricao = $("#editar_descricao").val();
        let imagem = $("#editar_imagem").val();
        let tipo = $("#editar_tipo").val();
        let faccao = $("#editar_faccao").val();
        let ativo = $("#editar_ativo").is(":checked") ? 1 : 0;
    
        $.ajax({
            type:"POST",
            url:"personagem-model.php",
            data:"acao="+acao+"&ID="+ID+"&nome="+nome+"&descricao="+descricao+"&imagem="+imagem+"&tipo="+tipo+"&faccao="+faccao+"&ativo="+ativo,
            success: function(msg){
                alert(msg);

                window.location.reload();
            }
        });
    });
});